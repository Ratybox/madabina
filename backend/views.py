...
def perform_pca(request):
    try:
        ...
        valid_pca_types = ['normalized', 'normalized_kaiser', 'homogeneous', 'homogeneous_kaiser', 'heterogeneous', 'heterogeneous_kaiser']
        if pca_type not in valid_pca_types:
            return Response({"error": f"Type de PCA non valide. Utilisez l'un des types suivants: {', '.join(valid_pca_types)}."}, 
                           status=status.HTTP_400_BAD_REQUEST)
...
def test_example(request):
    try:
        ...
        results = {
            ...
            'normalized_kaiser': pca(data, pca_type='normalized_kaiser'),
            'homogeneous': pca(data, pca_type='homogeneous'),
            'homogeneous_kaiser': pca(data, pca_type='homogeneous_kaiser'),
            'heterogeneous': pca(data, pca_type='heterogeneous'),
            'heterogeneous_kaiser': pca(data, pca_type='heterogeneous_kaiser')
        }
